...
from src.static.prompt import GENERATE_WITH_EXAMPLES_PROMPT
...
def generate_without_rag(state: StudentState, config: Optional[RunnableConfig] = None) -> StudentState:
    """RAG 없이 기본 생성 (폴백용)
    
    RAG 검색이 실패했거나 예시가 없을 때 사용하는 기본 생성 함수
    ...
    Returns:
        세특이 생성된 상태
    """
    # retrieved_examples를 빈 리스트로 설정하고 generate 호출
